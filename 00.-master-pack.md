...
| Name | Path | Layer | Used In | Directive | States | A11y Notes | Health | Owner Feature | Notes |
|------|------|-------|---------|---------------|--------|------------|--------|---------------|-------|
| FavoriteButton | `src/components/favorites/favorite-button.tsx` | feature | ProductView, ProductCard | Yes | isFavorite | Button aria-label | Locked | Favorites | Must call `useFavorites().toggle`; must NOT import server actions directly |
| WishlistClient | `src/components/favorites/wishlist-client.tsx` | feature | `/account/wishlist` page | Yes | loading | ProductCard grid | Healthy | Favorites | Renders wishlist; if filtering by `isFavorite`, depends on provider sync |
| FavoritesProvider | `src/components/favorites/favorites-provider.tsx` | provider | Root layout | Yes | favoriteIds, hydrated | - | Locked | Favorites | Favorites state, server sync, optimistic updates; single source for favorite mutations |
