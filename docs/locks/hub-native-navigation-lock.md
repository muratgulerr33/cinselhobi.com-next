...
- 2026-03-22 stabilizasyon notu: Hub -> category client navigation sonrası route contract korunarak category page tarafında `childCategories` verisi doğrudan ilgili client bileşenlerine verildi; `?sub=` slug'a çevrilmedi, wcId bazlı contract aynen kaldı
...
- `sub` (query param): Child category wcId (number)
  ...
  - 2026-03-22 stabilizasyonu: category page header/filter senkronu için `category-catalog-params-sync` içinde de mevcut child category listesi üzerinden wcId -> internal id dönüşümü yapılarak drawer başlangıç seçimi URL ile hizalandı
...
- [x] Hub -> category client navigation sonrası mobil header sağ üst filtre/sıralama ikonları ilk geçişte görünür kalır
- [x] Category page active sub filter chip'i gerçek child label gösterir; `Alt kategori 226` fallback'i görünmez
- [x] Drawer açıldığında başlangıç seçili alt kategori URL `?sub=` ile senkron gelir
...
9. From `/hub/erkek-ve-performans`, navigate to `/erkeklere-ozel?sub=226` → Verify mobile header controls visible, chip `Geciktiriciler`, drawer selection synced
10. From the same hub, navigate to `/erkeklere-ozel?sub=117` → Verify chip `Halka ve Kılıflar`, drawer selection synced
11. Known issue note: drawer `Description` / focus warning'leri secondary'dir; bu akıştaki ana root cause değildir
