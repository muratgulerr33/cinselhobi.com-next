# ChatGPT Master Knowledge Pack (DOC-01 … DOC-14)
> Generated on: 2025-12-30T23:51:48.430Z
> Manual update: 2026-01-15 — Checkout Stabilizasyonu bölümü kanıtlı hale getirildi
> Manual update: 2026-01-16 — Kredi Kartı Checkout (Mock) + PayTR (ENV-gated)
> Manual update: 2026-01-16 — Email Entegrasyonu (Sipariş) kilitlendi (PII hijyeni + inbox test + checklist)
> Manual update: 2026-01-16 — Hub Native Navigation (Merged Branch) lock doc v2 + MasterPack güncellemesi
> Manual update: 2026-03-22 — Hub -> category header controls + sub filter sync stabilize edildi
> Manual update: 2026-03-25 — PDP sticky action bar final composition + Tawk-safe Playwright doğrulaması eklendi
> Single source of truth. Do not invent routes/endpoints/fields. Use Unknown/Assumption when needed.

**Priority Order:** 02 > 04 > 05 > 08 > 03 > 09 > 06 > 07 > 10 > 11 > 12 > 01 > 13 > 14

## Quick Navigation
- Updates:
  - [2026-03-25 — PDP Sticky Bar + Tawk-safe Playwright](#update-2026-03-25-pdp-sticky-bar)
  - [2026-03-15 — Prod Runtime Reality Check](#update-2026-03-15-prod-reality)
  - [2026-01-19 — Incident / Restore + Repo Temizliği + DB Bakımı](#update-2026-01-19)
  - [2026-01-16 — Hub Native Navigation (Merged Branch)](#update-2026-01-16-hub-nav)
  - [2026-01-16 — Email Entegrasyonu (Sipariş) Kilit](#update-2026-01-16-email)
  - [2026-01-16 — PayTR / Kredi Kartı](#update-2026-01-16)
  - [2026-01-15 — Checkout Stabilizasyonu](#update-2026-01-15)

## Table of Contents
- [DOC-01: 01.project-brief.md](#doc-01)
- [DOC-02: 02.architecture-lock.md](#doc-02)
- [DOC-03: 03.routes-and-navigation-map.md](#doc-03)
- [DOC-04: 04.design-system.md](#doc-04)
- [DOC-05: 05.native-ux-rules.md](#doc-05)
- [DOC-06: 06.frontend-standards-2026.md](#doc-06)
- [DOC-07: 07.component-inventory.md](#doc-07)
- [DOC-08: 08.api-contracts-frontend.md](#doc-08)
- [DOC-09: 09.data-fetching-cache-rules.md](#doc-09)
- [DOC-10: 10.tasks-playbook.md](#doc-10)
- [DOC-11: 11.qa-test-matrix.md](#doc-11)
- [DOC-12: 12.release-prod-checklist.md](#doc-12)
- [DOC-13: 13.security-and-deps.md](#doc-13)
- [DOC-14: 14.ChatGPT-bridge-instructions.md](#doc-14)

---

<a id="update-2026-03-25-pdp-sticky-bar"></a>
## 2026-03-25 — PDP Sticky Bar + Tawk-safe Playwright

### A) Özet

- Mobile PDP sticky action bar final düzeni tek satır olarak kilitlendi: `[ birim fiyat ] [ spacer ] [ telefon ] [ WhatsApp ] [ Sepete Ekle ]`
- Sticky barda favorite kaldırıldı; favorite sistemi korunuyor
- Telefon ve WhatsApp secondary action'ları icon-only kaldı; primary CTA tek baskın aksiyon olarak `Sepete Ekle`
- `Sepete Ekle` butonunun kontrolsüz yatay büyümesi kaldırıldı; sağ blokta daha dengeli hizalandı
- Playwright doğrulamasında Tawk widget runtime davranışı değiştirilmeden, yalnız test context'inde overlay ignore/hide edilerek sticky bar kontrolleri stabilize edildi
- `.env.local` ile oynanmadı; Tawk key aç/kapa yapılmadı

### B) Kanıtlı Kararlar

- PDP sticky bar product-detail özel, one-off `ProductView` içinde kalır; shared favorite mimarisi buraya taşınmaz
- ProductCard üzerindeki kalp ve `/account/wishlist` akışı bu değişiklikten etkilenmez
- Tawk runtime görünürlüğü uygulama tarafında route + suppression kaynakları ile yönetilir; test tarafında vendor overlay blokajı görülürse Playwright context'inde gizlenebilir

**Evidence:** `src/components/product/product-view.tsx`, `src/components/integrations/tawk/tawk-widget-boot.tsx`, `src/components/integrations/tawk/tawk-visibility.ts`

---

<a id="update-2026-03-15-prod-reality"></a>
## 2026-03-15 — Prod Runtime Reality Check

### A) Verified observations

- Public canonical prod domain: `https://www.cinselhobi.com`
- Current hosting provider: Hetzner
- Current hosting model: self-hosted Linux VPS
- PM2 process adı: `cinselhobi-next`
- Current app working directory: `/var/www/cinselhobi/app`
- App port: `3000`
- Sunucuda `npm run build` başarılı geçti
- `pm2 restart` sonrası kullanıcı etkili bug düzeldi: filtre ve ikonlar çalıştı
- Current git remote: `github-cinselhobi:muratgulerr33/cinselhobi.com-next.git`
- `~/.ssh/config` içinde `github-cinselhobi` host alias tanımlı
- Doğru deploy key `~/.ssh/gh_cinselhobi_ed25519` olarak doğrulandı
- Deploy key label: `hetzner-prod-cinselhobi`
- `ssh -T github-cinselhobi` başarılı çalıştı
- `git pull origin main` başarılı çalıştı ve `Already up to date.` döndü
- PM2 env observation:
  - `AUTH_URL=https://cinselhobi.com`
  - `NEXTAUTH_URL=https://cinselhobi.com`
  - `PORT=3000`
- PM2 env'de görülmeyenler:
  - `NEXT_SERVER_ACTIONS_ENCRYPTION_KEY`
  - `NEXT_DEPLOYMENT_ID`
- Runtime observation only: PM2 loglarında tekrar eden `Failed to find Server Action. This request might be from an older or newer deployment.` mesajı görüldü

### B) Interpretation / documentation standard

- Current public/canonical prod anlatımı `https://www.cinselhobi.com` üzerinden yazılmalıdır.
- Runtime env observation ile canonical docs standardı karıştırılmamalıdır: PM2 env'de `AUTH_URL`/`NEXTAUTH_URL` non-www görülse de doküman standardı `www` olmalıdır.
- Current deploy/rollback anlatımında app directory `/var/www/cinselhobi/app`, PM2 process `cinselhobi-next`, port `3000` kullanılmalıdır.
- Current provider/current runtime anlatımında `Hetzner` + `self-hosted Linux VPS` birlikte kullanılabilir.
- Current git/SSH anlatımında `github-cinselhobi` alias'ı resolved fact olarak yazılabilir; private key içeriği veya fingerprint yazılmamalıdır.
- Other current docs içindeki prod runtime fact'ler bu bölüme göre yorumlanmalıdır.
- `12.release-prod-checklist.md` standalone dosyası bu repo snapshot'ında bulunmadı; DOC-12 içeriği master pack içinde mevcut. Standalone file status: Manual verification required.

---

<a id="update-2026-01-16-hub-nav"></a>
## 2026-01-16 — Hub Native Navigation (Merged Branch)

### A) Özet

- Hub native navigation implementasyonu tamamlandı: `/hub` (bento grid) + `/hub/[hubSlug]` (hero + chip/pill rail + quick look sheet)
- Chip/Pill rail pattern (story bubble yerine): h-11, rounded-full, initials + label, ellipsis
- Quick Look Sheet: lazy fetch (sheet açılınca), skeleton-first, 8 ürün limit, inStock=1 filter
- Hero images: 5/5 hub için mevcut (`public/images/hub/hero/${hubSlug}.webp`)
- "Tümünü Gör" URL contract: `/${parentSlug}?sub=${childWcId}` (kilitli)
- Single drawer rule: Vaul ile cart + quick look çakışması çözüldü
- Category cards grid kaldırıldı (sadece rail + sheet pattern)
- 2026-03-22 stabilizasyonu: Hub -> child category geçişinde mobil header sağ üst filtre/sıralama ikonlarının bazen görünmemesi, aktif filtre chip label'ının bazen `Alt kategori 226` fallback'ine düşmesi ve drawer başlangıç seçili alt kategori senkronunun bozulması düzeltildi
- Root cause: route geçişinde `HeaderContext` içindeki `categoryInfo` ve `catalogParams` kısa süreli reset/null kalıyordu; category page'in server'da zaten bildiği `childCategories` verisi ilgili client bileşenlerine doğrudan verilmediği için header controls görünürlüğü ve chip label çözümü timing'e bağlı kırılıyordu; ayrıca `category-catalog-params-sync` içinde `?sub=` wcId -> internal id dönüşümü eksikti
- Minimal fix: page-level `childCategories` verisi doğrudan `ActiveFiltersBar` ve `CategoryCatalogParamsSync`'e verildi; `header.tsx` sadece render shell görünürlüğünü stabilize edecek şekilde bırakıldı; `CatalogControls` props zinciri üzerinden yeniden mount/senkron çalışacak şekilde korundu
- Dokunulan dosyalar: `src/app/[slug]/page.tsx`, `src/components/catalog/active-filters-bar.tsx`, `src/components/catalog/catalog-controls.tsx`, `src/components/catalog/category-catalog-params-sync.tsx`, `src/components/layout/header.tsx`

**Evidence:** `docs/locks/hub-native-navigation-lock.md` (v2), `src/app/hub/page.tsx`, `src/app/hub/[hubSlug]/page.tsx`, `src/components/hub/category-bubble-rail.tsx`, `src/components/hub/category-quick-look-sheet.tsx`, `src/components/hub/hub-category-rail.tsx`, `public/images/hub/hero/` (5 files)

### B) Kilitli Kararlar

1. **Route Template:** `/hub` = Bento grid, `/hub/[hubSlug]` = Hero + Stream (category cards grid yok)
2. **Quick Category Rail:** Chip/Pill pattern (h-11, initials circle + label, ellipsis, horizontal scroll)
3. **Quick Look Fetch:** User-triggered (lazy), skeleton-first, limit=8, **inStock=1 filter (kilitli)**
4. **"Tümünü Gör" URL:** `/${parentSlug}?sub=${childWcId}` (contract kırılamaz)
5. **Empty Category Policy:** `policy: "hidden-if-empty"` + DB-side filtering (HAVING clause)
6. **Single Drawer Rule:** Vaul default behavior (cart + quick look çakışması yok)
7. **Hero Images:** 5/5 hub mevcut (naming: `${hubSlug}.webp`)

**Evidence:** `docs/locks/hub-native-navigation-lock.md` (section 2: Non-negotiables), `src/components/hub/category-quick-look-sheet.tsx` (line 64: inStock=1), `src/app/hub/[hubSlug]/page.tsx` (lines 47-55: empty category filter)

### C) Implementasyon Detayları

**Components:**
- `CategoryBubbleRail`: Chip/Pill rail (initials generation, ellipsis, scroll affordance)
- `HubCategoryRail`: Wrapper (state: selectedCategory, sheetOpen)
- `CategoryQuickLookSheet`: Bottom sheet (lazy fetch, skeleton, 8 products, inStock=1)
- `ActiveFiltersBar`: child label çözümünde page-level `childCategories` verisini birincil kaynak olarak kullanır; header context yalnız fallback olur
- `CategoryCatalogParamsSync`: `?sub=` wcId değerlerini mevcut child category listesi ile internal id'lere çevirip `HeaderContext` içine yazar
- `CatalogControls`: category route'ta render shell görünür kalır; seçili filtre state'i props zinciri üzerinden senkronize edilir, yeni bağımsız source of truth eklenmez

**Routes:**
- `/hub`: Bento grid (hub cards)
- `/hub/[hubSlug]`: Hero + Quick Category Rail (no category cards grid)

**Assets:**
- Hero images: `public/images/hub/hero/${hubSlug}.webp` (5/5 present)

**Evidence:** `src/components/hub/category-bubble-rail.tsx` (lines 77-125: chip/pill UI), `src/components/hub/category-quick-look-sheet.tsx` (lines 45-92: lazy fetch), `src/app/hub/[hubSlug]/page.tsx` (lines 72-100: hero section)

### D) Drift Düzeltmeleri (v1 → v2)

Lock doc v1'den v2'ye güncellemeler:
- Story bubbles → Chip/Pill rail (Türkçe label uyumu için)
- Hero images: 1/5 → 5/5 (tüm hub'lar için mevcut)
- Quick Look fetch: `inStock=1` parametresi eklendi (kilitli karar)
- Category Cards Grid: Kaldırıldı (sadece rail + sheet pattern)

**Evidence:** `docs/locks/hub-native-navigation-lock.md` (v2 updates), `dokuman güncelleme.md` (Parça 1/3, section 1.2: Story Bubble → Chip/Pill pivot)
...
**Guardrail:**
- `npm run guardrail:forbidden` (QA için çalıştırılmalı)
- `npm run category:lock` (QA için çalıştırılmalı)

**Smoke Tests:**
- Hub index loads (`/hub`)
- Hub detail loads with hero image (`/hub/[hubSlug]`)
- Chip/Pill rail renders
- Chip tap → sheet opens (skeleton → products, inStock=1)
- "Tümünü Gör" navigates correctly
- Empty categories hidden
- Single drawer rule works
- `/hub/erkek-ve-performans` → `/erkeklere-ozel?sub=226`: mobil header controls ilk geçişte görünür, chip `Geciktiriciler` yazar, drawer'da `Geciktiriciler` seçili gelir
- `/hub/erkek-ve-performans` → `/erkeklere-ozel?sub=117`: chip `Halka ve Kılıflar` yazar, drawer'da `Halka ve Kılıflar` seçili gelir
- Hard refresh / direct load: `?sub=` contract korunur, chip fallback'e düşmez, hydration mismatch kalmaz
- Known issue (secondary): drawer `Description` / focus warning'leri gözlendi, ancak bu bug'ın ana root cause'u değildir ve bu fix kapsamında çözülmedi

**Evidence:** `docs/locks/hub-native-navigation-lock.md` (section 15: DoD & QA Checklist)

---

<a id="update-2026-01-16-email"></a>
## 2026-01-16 — Email Entegrasyonu (Sipariş) Kilit

### A) Özet

- COD: Order sonrası hemen 2 mail (customer + admin)
- CC: PayTR callback sonrası (paid olduğunda) 2 mail
- EMAIL_ENABLED gate: `EMAIL_ENABLED !== "true"` ise email gönderilmez
- Best-effort pattern: Mail hatası siparişi bozmaz

**Evidence:** `docs/eposta-test-runbook.md`, `docs/eposta-dry-run-test-raporu-2b.md`, `docs/eposta-inbox-test-raporu-3a.md`, `docs/eposta-kilit-checklist.md`, `docs/eposta-kilit-ozet-raporu.md`, `src/lib/email/send.ts`, `src/lib/email/transport.ts`, `src/actions/checkout.ts` (lines 40-123: sendOrderEmails), `src/app/api/payments/paytr/callback/route.ts` (lines 131-210: email gönderimi)

### B) Test Kanıtları

- Dry-run (EMAIL_ENABLED=false) PASS (UI+DB): Sipariş oluşturuldu, email gönderilmedi, flow bozulmadı
- Inbox testi (EMAIL_ENABLED=true) PASS (2 inbox): Customer email (`l***@gmail.com`) ve admin email (`d***@cinselhobi.com`) inbox'a ulaştı

**Evidence:** `docs/eposta-dry-run-test-raporu-2b.md` (Order ID: `105c1fc8-22d9-407d-bd34-98e1d144a1c9`, EMAIL_ENABLED=false test), `docs/eposta-inbox-test-raporu-3a.md` (Order ID: `e1d68d9d-fff1-4fe9-b755-d355b48f48fe`, EMAIL_ENABLED=true test, 2 email inbox'a ulaştı)

### C) Güvenlik

- PII maskeli dokümantasyon: Test raporlarında email adresleri (`l***@gmail.com`, `d***@cinselhobi.com`) ve isimler (`E*** A***`) maskelenmiş
- SMTP_USER/PASS sadece process.env'den okunuyor (hardcode yok): `src/lib/email/transport.ts` (lines 11-12: `process.env.SMTP_USER`, `process.env.SMTP_PASS`)
- .env* dosyaları gitignore'da

**Evidence:** `docs/eposta-inbox-test-raporu-3a.md` (PII maskeli), `docs/eposta-kilit-ozet-raporu.md` (PII hijyeni tamamlandı, repo sızıntı kontrolü PASS), `src/lib/email/transport.ts` (process.env okuma, hardcode yok), `.gitignore` (.env* ignore)

### D) Email Akışları

#### COD (Kapıda Ödeme)
- Order oluşturulduktan hemen sonra `sendOrderEmails()` çağrılır
- 2 email gönderilir: `sendOrderConfirmationEmail()` (customer), `sendAdminNotificationEmail()` (admin)

**Evidence:** `src/actions/checkout.ts` (lines 143-153: COD akışı, line 153: sendOrderEmails çağrısı)

#### Credit Card (Kredi Kartı)
- Order oluşturulur ama email gönderilmez (satır 272-273: email gönderimi yapılmaz)
- PayTR callback sonrası (`/api/payments/paytr/callback`) `status === "success"` ve `paymentStatus === "paid"` ise 2 email gönderilir

**Evidence:** `src/actions/checkout.ts` (lines 272-273: email gönderimi yapılmaz notu), `src/app/api/payments/paytr/callback/route.ts` (lines 186-266: email gönderimi callback'te)

### E) ENV Key'leri

Kullanılan ENV key'ler (sadece isimler, değer yok):
- `EMAIL_ENABLED` - Email gönderimini açıp kapatma (`"true"` olmalı)
- `SMTP_HOST` - SMTP sunucu adresi (zorunlu, default yok)
...
**Evidence:** `docs/eposta-test-runbook.md` (section 2: ENV Key Listesi), `src/lib/email/transport.ts` (SMTP config), `src/lib/email/send.ts` (SMTP_FROM, ADMIN_NOTIFY_TO), `src/actions/checkout.ts` (AUTH_URL, NEXT_PUBLIC_BASE_URL)

---

<a id="update-2026-01-16"></a>
## 2026-01-16 — Kredi Kartı Checkout (Mock) + PayTR (ENV-gated)

### A) Özet

- Local'da mock ile CC akışı tamamlandı (success/fail/3DS senaryoları)
- PayTR altyapısı eklendi (ENV-gated: configured değilse UI disabled + callback 404)
- Orders tablosuna payment alanları eklendi (paymentStatus, paymentTransactionId, paymentProvider, paymentMetadata)
- Email timing: COD'da hemen, CC'de callback sonrası (paid status)

**Evidence:** `docs/2026-01-16-paytr-kredi-karti-checkout.md` (full documentation), `src/lib/payments/payment-provider.ts`, `src/lib/payments/mock-credit-card-provider.ts`, `src/lib/payments/paytr-provider.ts`, `src/actions/checkout.ts` (lines 19-22: getPayTRConfigStatus, lines 125-302: createOrderAction), `src/app/api/payments/paytr/callback/route.ts`, `src/db/schema.ts` (lines 142, 155-158: payment fields), `drizzle/0007_woozy_wendigo.sql`

### B) Contracts

- `createOrderAction` output artık union type: COD için `{ ok: true, orderId }`, CC için `{ ok: true, paytr: { iframeToken, orderId } }`
- `getPayTRConfigStatus` action eklendi: `{ configured: boolean }`
- PayTR callback route: `/api/payments/paytr/callback` (POST, auth yok, hash verify + idempotency)

**Evidence:** `src/actions/checkout.ts` (lines 148: COD return, lines 276-282: CC return, lines 19-22: getPayTRConfigStatus), `src/app/api/payments/paytr/callback/route.ts` (full route)

### C) Provider Layer

- `PaymentProvider` interface: abstraction layer
- `MockCreditCardProvider`: dev-only, 3 senaryo (success/fail/3DS)
- `PayTRProvider`: ENV isimleri (`PAYTR_MERCHANT_ID`, `PAYTR_MERCHANT_KEY`, `PAYTR_MERCHANT_SALT`), `isConfigured()` guard, `startCardPayment()` iframe token alır

**Evidence:** `src/lib/payments/payment-provider.ts`, `src/lib/payments/mock-credit-card-provider.ts`, `src/lib/payments/paytr-provider.ts` (lines 27-36: isConfigured, lines 59-64: fetch guard)

### D) UX / Güvenlik

- Prod'da CC sadece configured ise enabled (UI disable + "Yakında" mesajı)
- Secrets sadece server-side (client'da expose yok)
- Callback auth'suz ama hash verify + idempotency (paid ise tekrar update yok)

**Evidence:** `src/app/checkout/page.tsx` (lines 451-464: disabled logic), `src/lib/payments/paytr-provider.ts` (server-only), `src/app/api/payments/paytr/callback/route.ts` (lines 73-87: hash, lines 101-105: idempotency)

---

<a id="update-2026-01-19"></a>
## 2026-01-19 — Incident / Restore + Repo Temizliği + DB Bakımı

### A) Özet

- **2026-01-17 (gece)**: Cursor workspace bağlamı sıfırlandı; tam proje yedeği alındı (tar + Desktop'a `.tgz`)
- **2026-01-17-19**: Repo temizliği yapıldı (artefact silme, `.gitignore` düzeni, unused SVG kaldırma)
- **2026-01-18-19**: DB bakımı (Postgres container, boyut ölçümü, NextAuth tabloları prune+vacuum)
- **2026-01-19**: Git release kilidi (branch push, tag push: `backup-2026-01-17`, `backup-2026-01-17b`, `backup-2026-01-18`, `backup-2026-01-19`)

**Evidence:** `docs/hub dokuman güncelleme.md` (Parça 1/3, 2/3, 3/3), `git log --oneline` (commits: 36deea9, ef6276d, 10b84ed, a5c4f3e, f17d8af), `docker ps` (container: `cinselhobi_db`, image: `postgres:16`, port: `5432`), `git tag -l backup-*`

### B) Incident / Kriz Yönetimi

#### Cursor Workspace Reset
- **Belirti**: Cursor workspace bağlamı/sohbet geçmişi sıfır görünmeye başladı; repo'nun "geride kalmış" olabileceği endişesi
- **Aksiyon**: Tam proje yedeği alındı (offline geri dönüş garantisi)
  - Komut: `tar -czf ~/Desktop/cinselhobi.com-next_backup_$(date +%Y%m%d_%H%M%S).tgz cinselhobi.com-next`
  - Sonuç: Desktop'a timestamp'li `.tgz` dosyası oluşturuldu
- **Cursor güvenlik ayarı**: "Run everything" → "Ask everytime" moduna geçildi
- **Root cause**: Unknown (Cursor workspace root/plan yürütme davranışı veya UI state reset)
  - Doğrulama: Cursor workspace root path/logları ve `.cursor` config geçmişi incelenmeli

**Evidence:** `docs/hub dokuman güncelleme.md` (Parça 1/3, section 1: Timeline, section 5: Riskler), kullanıcı beyanı + Cursor workspace ekran görüntüsü

#### Sağlık Kontrolleri
- **Lint**: `npm run lint` → `0 errors, 114 warnings` (çoğu `no-unused-vars`, bazı `no-img-element`, hooks deps uyarıları)
- **Build**: `npm run build` → Başarılı (Compiled successfully, route listesi üretildi)
- **Typecheck**: `npm run typecheck` → Script yok (projede typecheck script'i tanımlı değil)

**Evidence:** `npm run lint` çıktısı (`✖ 114 problems (0 errors, 114 warnings)`), `npm run build` çıktısı (`✓ Compiled successfully…`), `npm error Missing script: "typecheck"`

### C) Repo & Dosya Temizliği

#### Artefact Temizliği (commit 36deea9)
- **Silinenler**: Rapor/CSV/export dosyaları (2514 satır silinme)
  - Örnekler: `CATEGORY_*.csv/.md`, `HUB_DISCOVERY_REPORT.md`, `RENK_DENETIM_RAPORU.md`, `exports.zip`, `final ... woo products ... .csv`, `mindmap-stok.md`
- **Commit**: `36deea98a5…` — "Keşfet navigasyonu + hub UI polish + categories redirect"

**Evidence:** `git log -1 --stat 36deea9` (2514 deletions, dosya listesi)

#### .gitignore Düzeni
- **commit ef6276d**: `.gitignore` güncellendi (build artifacts, local artifacts ignore)
- **commit 10b84ed**: Legacy klasörler (`old-products/`, `mcp-server/`) yerelde silindi ve ignore edildi; `.cursor plan` artefact'ı repo'dan kaldırıldı
- **commit f17d8af**: `.gitignore` tekrar satırları temizlendi (dedupe, 5 satır silindi)

**Evidence:** `git log --oneline ef6276d 10b84ed f17d8af`, `.gitignore` dedupe Python komutu

#### Unused Public SVG Kaldırma (commit a5c4f3e)
- **Silinenler**: `public/{file,globe,logo,next,vercel,window}.svg` (6 dosya)
- **Kontrol**: `rg -n "file\.svg|globe\.svg|..." src public` ile referans kullanım araması yapıldı
- **Commit**: `a5c4f3e` — "chore: remove unused public svg assets"

**Evidence:** `git log -1 --stat a5c4f3e` (6 deletions)

#### Snapshots Durumu
- **Kontrol**: `git ls-files | rg '^data/snapshots'` → çıktı yok (tracked değil)
- **Local**: `data/snapshots/sample` localde var (`categories.json`, `products_page_001.json`, `summary.json`)
- **Ignore**: `.gitignore` içinde `data/snapshots/` pattern'i sample'ı ignore ediyor
  - Doğrulama: `git check-ignore -v data/snapshots/sample/categories.json` → `.gitignore:48:data/snapshots/`

**Evidence:** `git ls-files | rg '^data/snapshots'` (boş çıktı), `ls -la data/snapshots/sample`, `git check-ignore -v data/snapshots/sample/categories.json`

### D) Build/Lint/Dev Uyarıları

#### Lint Durumu
- **Error**: 0
- **Warning**: 114 (çoğu scripts/ ve bazı unused var/import)
- **Sonuç**: Release blocker değil ama teknik borç

**Evidence:** `npm run lint` çıktısı (`✖ 114 problems (0 errors, 114 warnings)`)

#### Build Durumu
- **Sonuç**: Başarılı
- **Route listesi**: Üretildi (örn. `/hub`, `/hub/[hubSlug]`, `/categories`, `/checkout`)

**Evidence:** `npm run build` çıktısı (`✓ Compiled successfully… Finished TypeScript… Route (app) …`)

#### Dev Server Uyarıları
- **Uyarı 1**: "Cross origin request… allowedDevOrigins…" (LAN'dan erişim sırasında)
- **Uyarı 2**: "metadataBase property… not set…" (OG/Twitter image resolve için prod'da net domain verilerek kapatılabilir)

**Evidence:** `npm run dev` log çıktıları (uyarı satırları)

### E) DB (Postgres Docker + Bakım)

#### Postgres Container Bilgileri
- **Container**: `cinselhobi_db`
- **Image**: `postgres:16`
- **Port**: `5432` (expose: `0.0.0.0:5432->5432/tcp`)
- **Connection**: `DATABASE_URL="postgres://c***:c***@localhost:5432/c***"` (PII maskeli)

**Evidence:** `docker ps --format 'table ...' | rg -i 'postgres|5432'` (container listesi), `.env.local` içinde `DATABASE_URL` (maskeli)

#### DB Boyutu ve Tablolar
- **DB Size**: `10 MB` (Kanıt: `docker exec -it cinselhobi_db psql -U cinselhobi -d cinselhobi -c "select pg_size_pretty(pg_database_size(current_database()))"` → `db_size | 10 MB`)
- **Largest tables** (top 15):
  - `products`: ~1896 kB
  - `product_categories`: ~80 kB
  - `categories`: ~64 kB
  - (diğer tablolar)

**Evidence:** `docker exec -it cinselhobi_db psql -U cinselhobi -d cinselhobi -c "select relname, pg_size_pretty(pg_total_relation_size(relid)) as size from pg_catalog.pg_statio_user_tables order by pg_total_relation_size(relid) desc limit 15;"`

#### NextAuth Tabloları
- **Tablolar**: `session`, `verification_token`
- **Expires kolonları**: Her iki tabloda da `expires` kolonu mevcut
- **Schema doğrulama**: `docker exec -i cinselhobi_db psql -U cinselhobi -d cinselhobi -c "select table_name, column_name, data_type from information_schema.columns where table_name in ('session','verification_token') ..."` → 6 satırlık schema çıktısı

**Evidence:** `docker exec -i cinselhobi_db psql ...` (schema çıktısı)

#### DB Fit Kalma Bakımı (Expired Prune + VACUUM)
- **İşlem**: NextAuth tabloları için expired kayıtları silme + VACUUM/ANALYZE
- **Yöntem**: `docker exec -i cinselhobi_db psql ... <<'SQL' ... SQL` (heredoc)
- **Sonuç**: `DELETE 0`, `DELETE 0`, `VACUUM`, `VACUUM` (expired kayıt yoktu)
- **Kontrol**: `select 'session'..., count(*) ... union all ...` → `session 0`, `verification_token 0`

**VACUUM Transaction Hatası ve Çözümü:**
- **Hata**: `ERROR: VACUUM cannot run inside a transaction block` (tek `-c` içinde birden çok komut transaction'a girince)
- **Çözüm**: `VACUUM` ayrı statement/context ile çalıştırıldı (heredoc içinde, tek transaction'a sıkıştırmadan)

**Evidence:** `docker exec -i cinselhobi_db psql ... <<'SQL' ... SQL` (heredoc çıktısı), `ERROR: VACUUM cannot run inside a transaction block` (hata mesajı), son çalıştırmada `DELETE 0 ... VACUUM` (başarılı)

#### dbfit Helper Hikayesi
- **Girişim**: `.zshrc` içine heredoc'lu alias/function ekleme
- **Sorun 1**: Alias olarak eklenince terminal "heredoc>" moduna takıldı
- **Sorun 2**: Function ile tek `-c` içinde `VACUUM` çağrılınca transaction hatası
- **Çözüm**: `VACUUM` ayrı statement/context ile çalıştırıldı
- **Not**: Helper `.zshrc`'de tutuluyor; repo'ya commit edilmedi (`git status` clean)

**Evidence:** `type dbfit` (alias gösterdiğinde `heredoc>` prompt'u), `VACUUM cannot run inside a transaction block` (hata), son çalıştırmada `DELETE 0 ... VACUUM` (başarılı)

### F) Git Release Kilidi

#### Branch ve Remote Tracking
- **Branch**: `audit/restore-gap-20260105-142645`
- **Remote tracking**: `origin/audit/restore-gap-20260105-142645`
- **Push**: `git push -u origin audit/restore-gap-20260105-142645`

**Evidence:** `git branch -vv` (remote tracking bilgisi), `git push -u origin ...` çıktısı

#### Tags
- **Tag'ler**: `backup-2026-01-17`, `backup-2026-01-17b`, `backup-2026-01-18`, `backup-2026-01-19`
- **Force update**: `git tag -f backup-2026-01-19` (gerekirse)
- **Push**: `git push --tags` (tüm tag'ler remote'a gönderildi)

**Evidence:** `git tag -l backup-*`, `git push --tags` çıktısı (new tag listesi), `git show ... backup-2026-01-19`

#### Son Durum
- **Working tree**: Clean (`git status -sb` → clean)
- **Branch/remote**: Doğrulandı
- **Tag'ler**: Remote'da mevcut

**Evidence:** `git status -sb` (clean), `git branch -vv`, `git tag -l backup-*`

### G) Kararlar ve Kilitler

1. **Kriz anında öncelik sırası kilidi**: Yedek al → Proje çalışıyor mu (build) doğrula → Temizlik/commit (sonra)
2. **Artık tek kaynak DB**: Legacy `old-products/` ve `mcp-server/` artık kullanılmayacak; temizlendi/ignore edildi
3. **Prod'a giderken "local snapshot" gitmesin**: `data/snapshots/` ignore kuralı ile garanti altına alındı
4. **Cursor guardrail**: "Run Everything" yerine "Ask Everytime" moduna alınarak yeniden kriz çıkarma riski azaltıldı
5. **DB bakım standardı**: Host'ta `psql` yoksa "daima `docker exec … psql`" kullan
6. **Backup stratejisi**: "backup-YYYY-MM-DD" tag'leri ile milat noktaları; force update gerektiğinde `git tag -f`

**Evidence:** `docs/hub dokuman güncelleme.md` (Parça 1/3, section 4: Kararlar; Parça 2/3, section 4: Kararlar; Parça 3/3, section 4: Kararlar)

### H) Riskler / Notlar

- **Incident root cause**: Unknown (Cursor workspace root/plan yürütme davranışı veya UI state reset)
  - Doğrulama: Cursor workspace root path/logları ve `.cursor` config geçmişi incelenmeli
- **Sağlık testi bulgusu**: Build geçiyor ama lint warning'leri yüksek; bu "release blocker" değil ama teknik borç
- **Secret/PII hijyeni**: DB bağlantı string'inde credential göründü; dokümana maskeli geçirildi
- **Zsh heredoc/alias riski**: `.zshrc` içinde heredoc'lu alias tanımı terminali "heredoc>" moduna sokabiliyor
  - Guardrail: dbfit'i heredoc yerine "docker exec + ayrı komutlar" şeklinde fonksiyonlaştırmak
- **VACUUM transaction kuralı**: Tek `-c` içinde birden çok komut transaction'a girip VACUUM'u patlatabiliyor
  - Guardrail: `VACUUM` ayrı call veya heredoc ile doğru bağlam
- **Prod notları**:
  - `allowedDevOrigins` uyarısı sadece dev/LAN erişimde; prod deploy öncesi dev config ayrı değerlendirilmeli
  - `metadataBase` uyarısı OG/Twitter image resolve için prod'da net domain verilerek kapatılabilir

**Evidence:** `docs/hub dokuman güncelleme.md` (Parça 1/3, section 5: Riskler; Parça 2/3, section 5: Riskler; Parça 3/3, section 5: Riskler)

---

<a id="update-2026-01-15"></a>
## 2026-01-15 — Checkout Stabilizasyonu (Adres + Sipariş Sonrası)

### A) Davranış Kuralları (Kilit)

#### Guest checkout'ta "Adresi Kaydet"
- Auth gerektiren server action çağrılmaz.
- Address payload localStorage'a `"checkout_address_intent"` olarak yazılır.
- Kullanıcı `/login?callbackUrl=/checkout` ile auth'a gider.
- Amaç: Unauthorized / gereksiz Next-Action POST'u engellemek.

**Evidence:** `src/components/account/address-form.tsx` (lines 86-93: guest kontrolü ve intent kaydetme), `src/lib/checkout-address-intent.ts` (line 19: INTENT_KEY = "checkout_address_intent")

#### Login/Register sonrası intent tüketimi
- Checkout'a dönünce intent okunur.
- Authenticated ise address create action çalıştırılır.
- Başarılı olunca intent silinir.
- Duplicate riskine karşı:
  - Client tarafında "tek seferlik" lock/guard `sessionStorage` ile `checkout_address_intent_lock:${intent.createdAt}` key'i kullanılır (gerçek format: `checkout_address_intent_lock:${intent.createdAt}`).
  - Server tarafında address create idempotent davranır: `addAddressAction` aynı `userId` için `title + fullAddress + city + district + phone` kombinasyonu ile mevcut adresi kontrol eder; varsa yeni kayıt oluşturmaz, mevcut adresi döner.

**Evidence:** `src/components/checkout/checkout-address-intent-consumer.tsx` (line 41: lock key formatı `checkout_address_intent_lock:${intent.createdAt}`, lines 44-49: sessionStorage lock mekanizması), `src/actions/address.ts` (lines 36-50: idempotent kontrol - `title`, `fullAddress`, `city`, `district`, `phone` alanları ile match), `src/lib/checkout-address-intent.ts` (consumeCheckoutAddressIntent fonksiyonu)

#### Checkout'ta address list fetch
- Sadece authenticated iken çağrılır (`status !== "authenticated"` kontrolü).
- Multi-render/refetch spam'ini azaltmak için "run-once" guard `loadAddressesCalledRef` (useRef) kullanılır.

**Evidence:** `src/app/checkout/page.tsx` (lines 40, 66-77: authenticated guard ve loadAddressesCalledRef)

### B) Authenticated Checkout'ta "Adres ekledim ama refresh olmadan görünmüyor" Fix'i

- Address form başarılı olunca checkout'a bir callback ile yeni address döner (`onCreated` prop).
- Checkout page:
  - Address list state'ine "upsert (id'ye göre ekle/güncelle)" yapılır (`upsertAddress` fonksiyonu).
  - Yeni eklenen address otomatik seçilir.
- Sonuç: Refresh gerekmiyor; UI anında güncelleniyor.

**Evidence:** `src/components/account/address-form.tsx` (lines 53, 106-107: onCreated callback), `src/app/checkout/page.tsx` (lines 80-95: upsertAddress fonksiyonu, line 365: onCreated={upsertAddress})

### C) Order Success sonrası clearCart döngüsü (Maximum update depth) Fix'i

- Cart provider'daki `clear` fonksiyonu stabil olmalı (useCallback + functional update) ve zaten boşsa no-op olmalı.
- Order success ekranındaki clear-cart effect:
  - Tek sefer çalışacak şekilde guard'lanmalı (`didClearRef` useRef ile).
  - Zaten boş cart'ta clear tetiklenmemeli.
- Amaç: Back/forward yaptığında "Maximum update depth exceeded" hatasını bitirmek.

**Evidence:** `src/components/cart/cart-provider.tsx` (lines 81-86: clear fonksiyonu useCallback + no-op guard), `src/components/order/order-success-clear-cart.tsx` (lines 11, 14-25: didClearRef guard ve boş cart kontrolü)

### D) Network Debug Notu (Kafa Karışıklığı Önleme)

- `/checkout` için birden fazla network satırı görülebilir:
  - RSC GET'ler normal olabilir.
  - "Next-Action" header'lı POST olan gerçek server action çağrısıdır.
- Bu debug kuralını dokümana kısa ve net ekle.

### E) Manuel Regression Checklist (Acemi)

1. **(Auth) Checkout'ta ilk adres ekle:** Refresh yok, listede görünür, seçili, sipariş tamamla aktif.

2. **(Auth) 2. adres ekle:** Seçilebilir, sipariş tamamla devam eder.

3. **(Guest) Sepete ekle → checkout'ta adres kaydet → login'e redirect → login → checkout'a dön:** Adres listede görünür, duplicate yok.

4. **(Order success) Sipariş alındı → back (sepet boş) → forward:** Hata yok, loop yok.

5. **(Network) Guest iken Next-Action POST beklenmez; auth sonrası beklenir (debug notu).**

**Evidence:** `src/components/account/address-form.tsx` (lines 86-94: guest kontrolü, lines 102-108: authenticated checkout'ta `onCreated` callback), `src/app/checkout/page.tsx` (lines 80-95: `upsertAddress` fonksiyonu, line 365: `onCreated={upsertAddress}`), `src/components/checkout/checkout-address-intent-consumer.tsx` (line 41: lock key formatı), `src/actions/address.ts` (lines 36-50: idempotent match kriterleri), `src/components/checkout/checkout-address-intent-consumer.tsx` (lines 24-26: status !== "authenticated" guard), `src/lib/checkout-address-intent.ts` (line 20: `TTL_MS = 15 * 60 * 1000` constant, lines 48-52: TTL kontrolü - 15 dakikadan eski intent'ler ignore edilir)
...
Cinselhobi.com Next, erken bootstrap aşamasında verisi içeri alınmış ve bugün Next.js App Router ile çalışan modern bir frontend arayüzü sunan full-stack e-ticaret uygulamasıdır. Proje, native mobile app hissi veren dokunsal ve akıcı bir kullanıcı deneyimi hedefler.
...
- Self-hosted VPS üzerinde deployment
...
**Evidence:** `docs/01-frontend-project-doc-v1.md` (lines 1-35), `README.md`, `package.json`
...
- **React:** 19.2.3
- **TypeScript:** 5.x
- **Node.js:** Exact repo-locked version unresolved (`package.json` içinde `engines` yok, repo kökünde `.nvmrc` yok)
...
├── data/snapshots/             # Historical bootstrap snapshots
├── docs/                       # Documentation
└── docker-compose.yml          # PostgreSQL container
```

**Evidence:** `src/app/` (directory structure), `src/components/` (directory structure), `src/db/schema.ts`
...
**Evidence:** `drizzle.config.ts` (lines 4-5), `src/db/connection.ts` (lines 6-7)
...
- Current prod runtime için aktif katalog import/deploy entegrasyonu yoktur.

**Evidence:** `README.md`, `docs/04-deploy-prod-pm2.md`

### 8.3 Environment Keys (Frontend)
- `DATABASE_URL`: PostgreSQL connection string
- `AUTH_SECRET`: NextAuth.js secret
- `AUTH_URL`: NextAuth.js base URL (production). Current docs standard canonical: `https://www.cinselhobi.com`; runtime PM2 observation on 2026-03-15 was non-www.

**Evidence:** `drizzle.config.ts`, `src/auth.ts` (line 17), `README.md` (lines 72-84)
...
- Deployment process dokümante edilmiş (`docs/04-deploy-prod-pm2.md`)
...
2. **README:** `README.md` - Setup instructions, runtime notes
...
13. **API Routes:** `src/app/api/products/route.ts`, `src/app/api/search/route.ts` - Internal APIs
...
20. **Historical Bootstrap Note:** `docs/03-woo-import-guide.md` - Historical record marker
---
...
Bu doküman, Cinselhobi.com Next projesinin mimari kararlarını, değişmez kuralları ve yasakları tanımlar. Amaç: AI asistanların (Cursor/ChatGPT) yanlış değişiklik yapmasını önlemek ve tutarlı geliştirme sağlamak.
...
Bu doküman, Cinselhobi.com Next projesinin route yapısını, navigasyon haritasını ve sayfa organizasyonunu tanımlar. Amaç: AI asistanların (Cursor/ChatGPT) route kırmasını, yanlış link vermesini ve flow kaçırmasını engellemek.
...
Bu doküman, Cinselhobi.com Next projesinin design system'ini, UI bileşenlerini, stil rehberini ve tasarım kararlarını tanımlar. Amaç: AI asistanların (Cursor/ChatGPT) tutarlı UI/UX tasarımı yapmasını sağlamak ve tasarım kararlarını merkezi bir yerde toplamak.
...
- `--background`: `oklch(0.985 0.001 106.423)` (warm paper - Stone-50)
- `--foreground`: `oklch(0.216 0.006 56.043)` (koyu gri - Stone-900)
- `--card`: `oklch(1 0 0)` (saf beyaz - korunur, ürün fotoğrafları için kritik)
- `--card-foreground`: `oklch(0.216 0.006 56.043)` (Stone-900)
- `--popover`: `oklch(1 0 0)` (saf beyaz - korunur)
...
- `--sidebar`: `oklch(0.985 0.001 106.423)` (Stone-50)
- `--sidebar-foreground`: `oklch(0.216 0.006 56.043)` (Stone-900)
- `--sidebar-accent`: `oklch(0.97 0.001 106.424)` (Stone-100)
- `--sidebar-accent-foreground`: `oklch(0.216 0.006 56.043)` (Stone-900)
- `--sidebar-border`: `oklch(0.923 0.003 48.717)` (Stone-200)

**Dark Theme Variables:**
- `--background`: `oklch(0.147 0.004 49.25)` (warm dark - Stone-950)
- `--foreground`: `oklch(0.97 0.001 106.424)` (yumuşak beyaz - Stone-100, pure white yerine göz konforu için)
- `--card`: `oklch(0.216 0.006 56.043)` (Stone-900, mavi hue'dan kurtuldu)
- `--card-foreground`: `oklch(0.97 0.001 106.424)` (Stone-100)
- `--popover`: `oklch(0.216 0.006 56.043)` (Stone-900)
- `--popover-foreground`: `oklch(0.97 0.001 106.424)` (Stone-100)
- `--primary`: `#ff2357` (aynı)
- `--primary-foreground`: `oklch(0.985 0 0)` (beyaz yazı)
- `--secondary`: `oklch(0.268 0.007 34.298)` (Stone-800)
- `--secondary-foreground`: `oklch(0.97 0.001 106.424)` (Stone-100)
- `--muted`: `oklch(0.268 0.007 34.298)` (Stone-800)
- `--muted-foreground`: `oklch(0.553 0.013 58.071)` (Stone-500)
- `--accent`: `oklch(0.268 0.007 34.298)` (Stone-800)
- `--accent-foreground`: `oklch(0.97 0.001 106.424)` (Stone-100)
- `--destructive`: `oklch(0.704 0.191 22.216)` (daha açık kırmızı)
- `--border`: `oklch(0.97 0.001 106.424 / 10%)` (sıcak ince sınır)
- `--input`: `oklch(0.97 0.001 106.424 / 15%)`
- `--ring`: `oklch(0.41 0.159 10.272)` (focus ring)
- `--sidebar`: `oklch(0.216 0.006 56.043)` (Stone-900)
- `--sidebar-foreground`: `oklch(0.97 0.001 106.424)` (Stone-100)
- `--sidebar-accent`: `oklch(0.268 0.007 34.298)` (Stone-800)
- `--sidebar-accent-foreground`: `oklch(0.97 0.001 106.424)` (Stone-100)
- `--sidebar-border`: `oklch(0.97 0.001 106.424 / 10%)`
...
Tema, gözü yormayan sıcak nötr (Stone) eksenine çekildi. **Hedef: Warm Paper + Warm Dark**

- **Light mode:** Sayfa zemini "warm paper" (Stone-50: `oklch(0.985 0.001 106.423)`). **Kritik:** Light mode'da `--card` ve `--popover` token'ları **saf beyaz** (`oklch(1 0 0)`) kalır - ürün fotoğrafları beyaz arkaplanlı olduğu için "yama" görünmesini önlemek için.
- **Dark mode:** Zifiri siyah / mavi-tabanlı koyu yerine "warm dark" (Stone-950: `oklch(0.147 0.004 49.25)`). `--foreground` saf beyaz yerine yumuşak beyaz (Stone-100) kullanılır - göz konforu için.

Tüm nötr/surface token'ları Stone paletine göre güncellendi. Brand/semantic token'lar (primary, destructive, charts, ring) aynen korundu.

**Evidence:** `src/app/globals.css` (lines 14, 48: `:root --background` ve `.dark --background`, Stone OKLCH değerleri), `tailwind.config.ts` (theme extend yok, CSS variables kullanılıyor)
...
**Product Detail Page:**
- Sticky action bar: Bottom fixed, tek satır; sol blokta birim fiyat, sağ blokta telefon icon, WhatsApp icon ve en sağda primary `Sepete Ekle`
...
Bu doküman, Cinselhobi.com Next projesinin "native app hissi" veren UX kurallarını, dokunsal etkileşim standartlarını ve kullanıcı geri bildirimi pattern'lerini tanımlar. Amaç: AI asistanların (Cursor/ChatGPT) tutarlı, native-feel UX tasarımı yapmasını sağlamak ve UI/UX çizgisini korumak.
...
2. **AI Asistan Rehberi:** ChatGPT/Cursor'un "fazla tasarım uydurmasını" engellemek, mevcut UX pattern'lerini korumak
...
- **Hub Quick Look Sheet:** Hub detail sayfalarında chip/pill rail'den kategori tıklandığında bottom sheet açılır, lazy fetch (sheet açılınca), skeleton-first, 8 ürün limit, inStock=1 filter (kanıt: `src/components/hub/category-quick-look-sheet.tsx` lines 45-92: lazy fetch, line 64: inStock=1)
- **Single Drawer Rule:** Aynı anda sadece bir drawer açık olabilir (Vaul default behavior) - cart + quick look çakışması yok (kanıt: `src/components/ui/drawer.tsx`, `src/components/hub/category-quick-look-sheet.tsx`)
- **Pattern:** Bottom-up slide animasyon, `rounded-t-2xl` top corners, backdrop blur, `max-h-[85dvh]` (kanıt: `src/components/ui/drawer.tsx`, `src/components/app/mobile-bottom-nav.tsx` line 227, `src/components/hub/category-quick-look-sheet.tsx` line 100)

**Evidence:** `src/components/app/mobile-bottom-nav.tsx` (cart drawer), `src/components/ui/drawer.tsx` (drawer component), `src/components/account/address-form.tsx` (address form drawer), `src/components/hub/category-quick-look-sheet.tsx` (quick look sheet), `docs/locks/hub-native-navigation-lock.md` (section 9: Drawer Rules)
...
- **Locked sticky bar composition:** Tek satır mobil düzen `[ birim fiyat ] [ spacer ] [ telefon ] [ WhatsApp ] [ Sepete Ekle ]`
- **Primary CTA rule:** Tek baskın CTA `Sepete Ekle` olmalı; secondary support actions icon-only ve `h-11 w-11` seviyesinde kalmalı
- **Favorite rule:** Favorite sticky barda yer almaz; favorite akışı ProductCard ve `/account/wishlist` üzerinden korunur

**Evidence:** `src/components/app/mobile-bottom-nav.tsx` (lines 47-50, product detail hide), `src/components/product/product-view.tsx`
...
Bu doküman, Cinselhobi.com Next projesinin frontend kodlama standartlarını, best practice'lerini ve geliştirme kurallarını tanımlar. Amaç: AI asistanların (Cursor/ChatGPT) tutarlı, kaliteli ve bakımı kolay kod yazmasını sağlamak ve frontend geliştirme standartlarını merkezi bir yerde toplamak.
...
1. **AI Asistan Rehberi:** ChatGPT/Cursor'un geliştirme yaparken UX/UI, erişilebilirlik, performans ve kalite kapılarından sapmasını engeller
...
Bu doküman, Cinselhobi.com Next projesindeki tüm component'lerin envanterini, kullanım yerlerini ve bağımlılıklarını tanımlar. Amaç: AI asistanların (Cursor/ChatGPT) gereksiz component yazmasını engellemek ve mevcut component'leri doğru şekilde kullanmasını sağlamak.
...
| Name | Path | Layer | Used In | Directive | States | A11y Notes | Health | Owner Feature | Notes |
|------|------|-------|---------|---------------|--------|------------|--------|---------------|-------|
| ProductView | `src/components/product/product-view.tsx` | feature | `/urun/[slug]` page | Yes | descriptionExpanded, isAdded, activeIndex | HeaderTitle, FavoriteButton, image alt | Healthy | Product | Main product detail view, gallery, sticky action bar; mobile sticky composition is unit price on the left and phone + WhatsApp + `Sepete Ekle` on the right; sticky favorite yok |
...
Bu doküman, Cinselhobi.com Next projesindeki frontend'in konuştuğu tüm API sözleşmelerini "tek doğru kaynak" olarak tanımlar. Amaç: AI asistanların (Cursor/ChatGPT) uydurma endpoint/alan üretmesini engellemek ve mevcut API contract'larını doğru şekilde kullanmasını sağlamak.
...
- Historical bootstrap import script API çağrıları (frontend'den kullanılmıyor)
...
Bu doküman, Cinselhobi.com Next projesinin veri getirme, cache/ISR, client vs server fetch, mutation/optimistic UI ve invalidation kurallarını tanımlar. Amaç: AI asistanların (Cursor/ChatGPT) yanlış cache, yanlış fetch yeri ve gereksiz client state üretmesini engellemek ve tutarlı veri yönetimi sağlamak.
...
- Hub Quick Look Sheet (user-triggered: sheet açılınca fetch, skeleton-first, limit=8, inStock=1)
...
**Evidence:** `src/components/catalog/load-more-grid.tsx` (lines 46-111: `useEffect` + `fetch`), `src/components/search/search-overlay.tsx` (lines 130-174: debounced search fetch), `src/components/product/product-detail-page.tsx` (lines 70-124: `useEffect` + `fetch` with AbortController), `src/components/hub/category-quick-look-sheet.tsx` (lines 45-92: lazy fetch on sheet open, line 64: inStock=1 filter)
...
Bu doküman, ChatGPT → Cursor akışını "tek doğru görev formatı" ile standartlaştıran task yönetimi playbook'udur. Amaç: Görevlerin daha az hatayla, daha hızlı, daha deterministik uygulanmasını sağlamak.
...
1. **Task Format Standardizasyonu:** ChatGPT'nin Cursor'a verdiği task format'ı tek doğru formata sabitler
2. **Uydurma Engelleme:** AI asistanların (Cursor/ChatGPT) uydurma endpoint/alan/component üretmesini engeller
...
## 2. Operating Model (ChatGPT ↔ Cursor ↔ Murti)
...
**ChatGPT (Beyin - Plan, Risk, Prompt):**
...
1. **Uydurma endpoint/alan yok** - `08.api-contracts-frontend.md` tek kaynak, yeni endpoint/alan eklemek için doküman güncellenmeli (kanıt: `08.api-contracts-frontend.md` (Breaking-Change Rules): "ChatGPT/Cursor uydurma endpoint/alan yazamaz")
...
ChatGPT'nin her cevapta Cursor'a vereceği standart format:
...
Bu doküman, Cinselhobi.com Next projesinin QA test matrisini, kritik akış test senaryolarını, erişilebilirlik kontrol listelerini ve release gate kriterlerini tanımlar. Amaç: Murti'nin "ChatGPT ile kontrol/test" rolünü standardize etmek ve her release öncesi tutarlı kalite kontrolü sağlamak.
...
### SMK-02A: Product Detail Sticky Bar Alignment
- **Steps:**
  1. `/urun/[slug]` sayfası mobile viewport ile açılır
  2. Sticky bar görünümü kontrol edilir
  3. Gerekirse Playwright test context'inde Tawk overlay ignore/hide edilir
- **Expected:**
  - Birim fiyat solda görünür
  - Telefon icon görünür
  - WhatsApp icon görünür
  - `Sepete Ekle` en sağda görünür
  - Sticky bar tek satır kalır
  - Tawk overlay Playwright doğrulamasını bloklamaz
  - Tawk runtime davranışı ve `.env.local` bu test için değiştirilmez
- **Evidence:** `src/components/product/product-view.tsx`, `src/components/integrations/tawk/tawk-widget-boot.tsx`, `src/components/integrations/tawk/tawk-visibility.ts`
...
- **Expected:**
  ...
  - Opsiyonel (EMAIL_ENABLED=true ortamında): Sipariş sonrası 2 email inbox'a düşer (customer + admin)
- **Evidence:** `03.routes-and-navigation-map.md` (section 6.1), `05.native-ux-rules.md` (section 6.3), `src/app/checkout/page.tsx` (empty cart redirect, loading state, submit button), `docs/eposta-test-runbook.md`, `docs/eposta-inbox-test-raporu-3a.md` (EMAIL_ENABLED=true test, 2 email inbox'a ulaştı), `src/lib/email/send.ts` (sendOrderConfirmationEmail, sendAdminNotificationEmail)
...
- **Steps:**
  ...
  6. Admin email inbox kontrol edilir
- **Expected:**
  - Order success sayfası gösterilir
  - Customer email inbox'a ulaştı (Subject: `Sipariş Onayı - #<ORDER_ID>`)
  - Admin email inbox'a ulaştı (Subject: `Yeni Sipariş - #<ORDER_ID>`)
- **Prerequisites:**
...
| `/urun/[slug]` | ProductView, HeaderTitle, FavoriteButton, Sticky action bar (birim fiyat + telefon + WhatsApp + `Sepete Ekle`) | Loading (`loading.tsx`), Error (notFound) | Back button, Bottom nav hidden | `src/app/urun/[slug]/page.tsx`, `src/app/urun/[slug]/loading.tsx`, `05.native-ux-rules.md` (section 12.1) |
...
- [ ] SMK-02: Search → Product Detail
- [ ] SMK-02A: PDP sticky action bar hizası (birim fiyat + telefon + WhatsApp + `Sepete Ekle`)
...
- [ ] `createOrderAction` contract güncel (COD + CC/paytr branch) ve checkout UI uyumlu
- [ ] PayTR configured değilken CC disabled, createOrderAction error döner
- [ ] Callback configured değilken 404 dönüyor (OK değil)
...
- [ ] **Redeploy previous release (self-hosted VPS):**
  - Sunucuya SSH ile gir
  - Uygulama dizininde (`/var/www/cinselhobi/app`) `git` ile önceki stabil commit/tag'e dön (hangi tag/commit kullanıldığı Unknown ise not düş)
...
- [ ] **Hotfix revert (self-hosted):**
  ...
  - Deploy dokümanındaki standart deploy akışını tekrar çalıştır (`git pull origin main`, `npm ci`, `npm run build`, `pm2 restart cinselhobi-next`)
...
**Evidence:** `01.project-brief.md` (Goals: self-hosted VPS), `docs/04-deploy-prod-pm2.md` (deploy/pm2 akışı), `12.release-prod-checklist.md` (Rollback hazırlığı)

**2026-03-15 operational resolution note:** Aynı gün içindeki önceki `ssh: Could not resolve hostname github-cinselhobi` blokajı, `github-cinselhobi` alias / deploy key düzeltmesi sonrası çözüldü. Current state: `ssh -T github-cinselhobi` ve `git pull origin main` başarılı.
...
- [ ] **Email issue varsa hızlı kapatma:** `EMAIL_ENABLED=false` yaparak email gönderimini devre dışı bırak (best-effort pattern sayesinde sipariş oluşturma normal çalışmaya devam eder)

**Evidence:** `02.architecture-lock.md` (section 2: Non-Negotiables), `10.tasks-playbook.md` (rollback stratejisi), `docs/eposta-kilit-checklist.md` (section 7: Rollback - EMAIL_ENABLED=false ile hızlı kapatma), `src/lib/email/send.ts` (EMAIL_ENABLED check, best-effort pattern)
...
- [ ] SMK-01: Home → Category → Product → Add to Cart (toast + button state)
- [ ] SMK-02: Search → Product Detail
- [ ] SMK-02A: PDP sticky action bar hizası (tek satır, CTA sırası doğru)
- [ ] SMK-03: Cart → Checkout → Order Success (boş sepet kontrolü, loading state)
...
- [ ] PDP: Sticky action bar'da birim fiyat solda, telefon ve WhatsApp icon button'ları CTA'nin solunda, `Sepete Ekle` en sağda
- [ ] PDP: Sticky action bar tek satır kalıyor, CTA kontrolsüz büyümüyor
- [ ] Playwright doğrulamasında Tawk overlay test context'inde ignore/hide edilebiliyor; runtime Tawk davranışı değiştirilmiyor
...
- [ ] **Rollback mi hotfix mi?** Karar verildi
  - S0/S1: Rollback (bkz. 12.1 — Redeploy previous release (self-hosted VPS + PM2))
...
- `SMTP_FROM` - Gönderen email adresi (default: `"Destek <destek@cinselhobi.com>"`)
- `ADMIN_NOTIFY_TO` - Admin bildirim email adresi (default: `"destek@cinselhobi.com"`)
- `AUTH_URL` - Base URL (email link'leri için, fallback: `NEXT_PUBLIC_BASE_URL` veya `"https://cinselhobi.com"`). Current docs standard canonical: `https://www.cinselhobi.com`; runtime PM2 observation on 2026-03-15 was non-www.
- `NEXT_PUBLIC_BASE_URL` - Public base URL (fallback, email link'leri için). Current docs standard canonical: `https://www.cinselhobi.com`.
...
2. **Server-Only Secrets**: `AUTH_SECRET`, `DATABASE_URL` gibi secret'lar sadece server-side kullanılır
...
### Email (SMTP/Nodemailer) — Secrets & PII Rules
...
- `ADMIN_NOTIFY_TO` - Admin bildirim email adresi (default: `"destek@cinselhobi.com"`)
- `AUTH_URL` - Base URL (email link'leri için, fallback: `NEXT_PUBLIC_BASE_URL` veya `"https://cinselhobi.com"`). Current docs standard canonical: `https://www.cinselhobi.com`; runtime PM2 observation on 2026-03-15 was non-www.
- `NEXT_PUBLIC_BASE_URL` - Public base URL (fallback). Current docs standard canonical: `https://www.cinselhobi.com`.

**Kurallar:**
1. `.env*` dosyaları git'e girmez (gitignore'da)
2. Loglarda secret yok: `console.error` ve `console.log` içinde SMTP_USER/SMTP_PASS değerleri loglanmaz
3. Test raporları PII maskeli olmalı: Email adresleri (`l***@gmail.com` formatı), isimler (`E*** A***` formatı)
4. SMTP_USER/PASS sadece `process.env`'den okunuyor (hardcode yok)

**Evidence:** `src/lib/email/transport.ts` (lines 11-12: `process.env.SMTP_USER`, `process.env.SMTP_PASS`, hardcode yok), `src/lib/email/send.ts` (EMAIL_ENABLED check, SMTP_FROM, ADMIN_NOTIFY_TO), `docs/eposta-inbox-test-raporu-3a.md` (PII maskeli), `docs/eposta-kilit-ozet-raporu.md` (PII hijyeni tamamlandı, repo sızıntı kontrolü PASS), `.gitignore` (.env* ignore)
...
**Evidence:** `package.json` (scripts: dev, build, start, lint, db:generate, db:migrate - audit yok)
...
17. Historical bootstrap import script - archived workflow reference only
...
docker exec -i cinselhobi_db psql -U cinselhobi -d cinselhobi -c "
